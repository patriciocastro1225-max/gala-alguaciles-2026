...
async function loadSeats(attendeeId:string|undefined, fallback:Recipient):Promise<Seat[]>{
 ...
 if(!url||!key)return [{seat_number:1,guest_name:fallback.name,guest_type:"Titular",qr_code:fallback.qr}];
 try{const r=await fetch(`${url}/rest/v1/rpc/get_attendee_seats`,{method:"POST",headers:{apikey:key,Authorization:`Bearer ${key}`,"Content-Type":"application/json"},body:JSON.stringify({p_attendee_id:attendeeId})});if(r.ok){const rows=await r.json();if(Array.isArray(rows)&&rows.length)return rows;}}catch{}
 return [{seat_number:1,guest_name:fallback.name,guest_type:"Titular",qr_code:fallback.qr}];
}
...
export async function POST(request:NextRequest){
 ...
 for(const person of body.recipients.slice(0,50)){
  ...
  const text=body.message.replaceAll("[Nombre]",person.name).replaceAll("[Mesa]",person.table).replaceAll("[Círculo]",person.circle).replaceAll("[QR]",seats.length>1?"Sus códigos QR personales se encuentran a continuación":"Su código QR personal se encuentra a continuación");
  ...
  const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({from,to:[person.email],subject:body.subject,html})});const payload=await response.json();results.push({attendee_id:person.attendee_id??null,email:person.email,ok:response.ok,id:payload.id,error:payload.message});
 }
