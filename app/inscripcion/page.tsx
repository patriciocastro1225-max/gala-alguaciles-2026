...
export default function RegistrationPage() {
  ...
  async function next() {
    ...
    try {
      ...
      if (!result) { result = await registerAttendee({ ...form, attendance_status: "Confirmado", payment_status: "Pendiente", companion_name: form.has_companion === "Sí" ? form.companion_name : "", notes }); setRegistrationResult(result); newRegistration = true; }
      if (newRegistration) await notifyOrganizers({ type: "registration", registration_code: result.registration_code, portal_token: result.portal_token, full_name: form.full_name, email: form.email, phone: form.phone, circle_name: form.circle_name, companion_name: form.has_companion === "Sí" ? form.companion_name : "", people, amount: totalAmount, payment_method: "TRANSFERENCIA · COMPROBANTE PENDIENTE DE VALIDACIÓN" });
      if (receiptFile) {
  ...
  function changeCompanion(value: string) { update("has_companion", value); if (value === "No") setForm((c:any)=>({...c,has_companion:"No",companion_name:"",companion_email:"",companion_phone:"",companion_rut:"",companion_is_alguacil:"No",companion_circle_name:"",companion_position:"",companion_dietary_notes:""})); }
