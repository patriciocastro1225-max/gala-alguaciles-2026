...
export default function RegistrationPage() {
  ...
  const [paymentChoice, setPaymentChoice] = useState<"Pendiente" | "Transferencia">("Pendiente");
  ...
  function validateCurrentStep() {
    if (step === 0 && (!form.full_name.trim() || !form.email.trim() || !form.phone.trim())) {
      setError("COMPLETE NOMBRE, CORREO Y TELÉFONO."); return false;
    }
    if (step === 2 && form.has_companion === "Sí" && !form.companion_name.trim()) {
      setError("INGRESE EL NOMBRE COMPLETO DEL ACOMPAÑANTE."); return false;
    }
    if (step === 4 && paymentChoice === "Transferencia" && !receiptFile) {
      setError("DEBE ADJUNTAR EL COMPROBANTE DE TRANSFERENCIA."); return false;
    }
    if (step === 5 && !form.consent) {
      setError("DEBE ACEPTAR LA POLÍTICA DE PRIVACIDAD Y EL TRATAMIENTO DE DATOS."); return false;
    }
  ...
  async function next() {
    ...
    try {
      ...
      if (paymentChoice === "Transferencia" && receiptFile) {
        await uploadPaymentReceipt(receiptFile, result.attendee_id, result.portal_token);
      }

      router.push(`/inscripcion/confirmacion?code=${encodeURIComponent(result.registration_code)}&token=${encodeURIComponent(result.portal_token)}&payment=${paymentChoice === "Transferencia" ? "revision" : "pendiente"}`);
    } catch (err) {
    ...
    <section className="registrationShell">
      ...
      <article className="registrationCard">
        ...
        <div className="guestFormGrid">
          ...
          {step === 4 && <section className="fullField companionRegistration"><div className="companionHeading"><p className="guestEyebrow">PAGO</p><h2>{paymentChoice === "Transferencia" ? "TRANSFERENCIA REALIZADA" : "PAGO PENDIENTE"}</h2><p>LA CONDICIÓN INVITACIÓN SOLO PUEDE SER ASIGNADA POR LA ADMINISTRACIÓN.</p></div><div className="guestFormGrid">
            <label className="fullField">SELECCIONE SU SITUACIÓN<select value={paymentChoice} onChange={(e) => { const value = e.target.value as "Pendiente" | "Transferencia"; setPaymentChoice(value); setReceiptFile(null); setError(""); }}><option value="Pendiente">PENDIENTE / PAGAR MÁS ADELANTE</option><option value="Transferencia">YA PAGUÉ POR TRANSFERENCIA</option></select></label>
            {paymentChoice === "Pendiente" && <div className="fullField"><p><strong>PAGAR AHORA</strong></p><p>PRÓXIMAMENTE PODRÁ PAGAR EN LÍNEA CON TARJETA O TRANSFERENCIA. EL SISTEMA ACTUALIZARÁ AUTOMÁTICAMENTE SU ESTADO A PAGADO.</p><button type="button" className="guestPrimary" disabled>PAGO EN LÍNEA · PRÓXIMAMENTE</button></div>}
            {paymentChoice === "Transferencia" && <div className="fullField"><p><strong>SUBIR COMPROBANTE DE TRANSFERENCIA *</strong></p><p>FORMATOS PERMITIDOS: PDF, JPG, JPEG O PNG. TAMAÑO MÁXIMO: 5 MB.</p><input type="file" accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png" onChange={(e) => chooseReceipt(e.target.files?.[0] ?? null)} />{receiptFile && <p><strong>ARCHIVO SELECCIONADO:</strong> {receiptFile.name.toUpperCase()}</p>}<p>EL PAGO QUEDARÁ COMO <strong>PENDIENTE DE VALIDACIÓN</strong> HASTA QUE EL COMITÉ REVISE EL COMPROBANTE.</p></div>}
          </div></section>}
